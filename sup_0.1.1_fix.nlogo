...
to go-once
  ;; 新生儿年龄为-1，年轻人年龄为0，老人年龄为1，年龄为2的会自然死亡
  ;; setupPop之后，年龄段为-1, 0
  ;; 如果是来自上一轮，那么年龄段为-1,0,1

  aging
  ;; aging过后，年龄层次为(0,1,2)

  ;; 最老一代人死亡(age = 2), 余下年龄段为(0,1)
  die_too_old
  ...
  ;; 人口控制（再抽样）
  ;; 此时老人的任务已经全部完成，直接控制中年人的人口
  popCtrl

  youngAdj

  ;; 生育小孩，年龄为 -1，此时年龄层为-1,0,1
  raisingChildren
...
to oldAgeSup
  ask turtles with [age = 0][
    ...
    if sup > disp_income [
      set sup disp_income
      set sup_rate sup / wage
    ]

    set disp_income disp_income - sup
...
to popCtrl
  let cc count turtles with [age = 0]
  if(cc > num_pop ) [
    ask n-of (cc - num_pop) turtles with [age = 0] [die]
  ]
end
...
to youngSaving
  ask turtles with [age = 0][
    ...
    if asset > disp_income [
      set asset disp_income
      set saving_rate asset / wage
    ]
    set c1  (disp_income - asset )


    if c1 < 0  [
      user-message (word asset "," disp_income "," c1)
...
to allocation
  ...
  ask turtles with [age = 1][
    ;; set asset Return_of_UnitCaptial * asset ;;完全折旧的版本

    set asset (1 + Return_of_UnitCaptial) * asset ;; 不折旧的版本
  ]
...
to raisingChildren
  ask turtles with [age = 0 ][
    ...
    let n (randomInt (1 + reproduction_rate))
    let total_cost  wage * (n * children-cost)

    if total_cost > disp_income [
      set n floor (disp_income / (wage * children-cost))
      set total_cost  wage * (n * children-cost)
    ]

    set disp_income disp_income - total_cost
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot min list Return_of_UnitCaptial 10"
...
"pen-2" 1.0 0 -2674135 true "" "plot max list 0 (mean_Saving_Rate - sd_saving_rate)"
...
"pen-2" 1.0 0 -2674135 true "" "plot max list 0 (mean_sup_rate - sd_sup_rate)"
"pen-3" 1.0 0 -955883 true "" "plot 0"
...
## sup_0.1.1_fix
1. move raisingChildren before saving
2. fix all



## sup_0.1.1 
1. cost of children -> doesnt change anything
